...
A. *Time-window truncation in the glass cut-off zone 1250–1265 nm — the issue that limits trust.*  final3's period puts the
   (±1,0) diffraction order in glass at cut-off at n_glass·P = 1251 nm; the associated Rayleigh-anomaly mode at ≈1257 nm decays
   with τ ≳ 1 ps, and the less lossy hot ITO damps it even less (probe end/peak 2.3 % at 300 K → 4.7 % at 6000–8000 K after
   600 fs, still 3.1 % after 900 fs).  The rectangular DFT window therefore leaves (i) a ripple of period ≈ 1/T_run on every
   spectrum (≈ 18 nm at the campaign's 300 fs, ≈ 13 nm at 400 fs, ≈ 9 nm at 600 fs, ≈ 6 nm at 900 fs), strongest near 1257 nm
   and growing with Te (±0.05 in T at 8000 K), (ii) an energy non-closure that appears in 1 − R − T but not in A_ITO, and
   (iii) a 1257-nm feature that is narrower than the window resolution and keeps sharpening with the window (8000 K: A_ITO peak
   0.40 at 600 fs → 0.54 at 900 fs).  Window sensitivity (`diag_time_convergence.png`, SUMMARY.md §3): 8000 K, 600 → 900 fs,
   raw max |ΔT| 0.14 inside the zone / 0.075 outside; after a 9-nm boxcar (one 600-fs ripple period) 0.055 inside / **0.017
   outside** (A_ITO 0.009 outside) — i.e. the band-averaged physics outside 1250–1265 nm is converged to ≈ 0.02 while the
   ripple and the cut-off feature are not.  Propagated to the feedback figure (T_eff computed from the raw table vs the 9-nm
   ripple-averaged table, `diag_smooth9nm_*`): max |ΔT_eff| 0.037 at 1244 nm (curve swing 0.37 → 0.59), 0.026 at 1258 nm,
   0.006 at 1274 nm; 0.06 anywhere outside the zone.  Rows of the T(λ,Te) table inside 1250–1265 nm, and T_eff at pump
   wavelengths there, should be read as truncation-limited; elsewhere the curves carry a ≈ ±0.03–0.06 ripple uncertainty that
   grows with Te.  Converging the ripple away would need runs of a few ps (τ ≳ 1 ps), ≈ 5× the present cost; the campaign's
   own 300-fs spectra carry the same effect with a longer ripple period, and the campaign flagged this zone (RUN_SUMMARY §4, §8).
